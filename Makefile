...
teapot:
	...
	docker build -t teapot-server:$(COMMIT_HASH) \
	    -t docker.pkg.github.com/martinsirbe/go-teapot-server/teapot-server:$(COMMIT_HASH) -f Dockerfile .
...
.PHONY: publish
publish:
	docker push docker.pkg.github.com/martinsirbe/go-teapot-server/teapot-server:$(COMMIT_HASH)
